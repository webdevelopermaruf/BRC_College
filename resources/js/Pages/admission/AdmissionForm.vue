<script setup>
import { form, apply } from '../../Store/AdmissionForm'
import { checkAndSetPresentData, setPresentData } from './admission'
import { ref } from 'vue';
...
function preview(index) {
    try {
        previewImg.value = URL.createObjectURL(form.upload.student[index]);
    } catch (error) {
        previewImg.value = '';
    }
}
                    ...
                    <div class="overview-contents">
                        ...
                        <div class="item upload hide flex-column">
                            <div class="row w-100">
                                ...
                                <div class="col-lg-4 col-12 input_group">
                                    <label> পাসপোর্ট সাইজের ছবি <b style="cursor: pointer;" data-bs-toggle="modal"
                                            @click="preview(0)" data-bs-target="#exampleModal">👁‍🗨</b>
                                    </label><br>
                                    <input type="file" @input="($event) => {
                                        form.upload.student[0] = ($event.target.files[0])
                                    }" />
                                </div>
                                <div class="col-lg-4 col-12 input_group">
                                    <label> পিতার আইডি কার্ডের ছবি <b style="cursor: pointer;" data-bs-toggle="modal"
                                            @click="preview(1)" data-bs-target="#exampleModal">👁‍🗨</b>
                                    </label><br>
                                    <input type="file" @input="($event) => {
                                        form.upload.student[1] = ($event.target.files[0])
                                    }" />
                                </div>
                                <div class="col-lg-4 col-12 input_group">
                                    <label> মাতার আইডি কার্ডের ছবি <b style="cursor: pointer;" data-bs-toggle="modal"
                                            @click="preview(2)" data-bs-target="#exampleModal">👁‍🗨</b>
                                    </label><br>
                                    <input type="file" @input="($event) => {
                                        form.upload.student[2] = ($event.target.files[0])
                                    }" />
                                </div>
                                <div class="col-lg-4 col-12 input_group">
                                    <label> প্রশংসাপত্র <b style="cursor: pointer;" data-bs-toggle="modal"
                                            @click="preview(3)" data-bs-target="#exampleModal">👁‍🗨</b>
                                    </label><br>
                                    <input type="file" @input="($event) => {
                                        form.upload.student[3] = ($event.target.files[0])
                                    }" />
                                </div>
                                <div class="col-lg-4 col-12 input_group">
                                    <label> একাডেমিক ট্রান্সক্রিপ্ট <b style="cursor: pointer;" data-bs-toggle="modal"
                                            @click="preview(4)" data-bs-target="#exampleModal">👁‍🗨</b>
                                    </label><br>
                                    <input type="file" @input="($event) => {
                                        form.upload.student[4] = ($event.target.files[0])
                                    }" />
                                </div>
                                <div class="col-lg-4 col-12 input_group">
                                    <label> এস. এস. সি. এডমিট কার্ড <b style="cursor: pointer;" data-bs-toggle="modal"
                                            @click="preview(5)" data-bs-target="#exampleModal">👁‍🗨</b>
                                    </label><br>
                                    <input type="file" @input="($event) => {
                                        form.upload.student[5] = ($event.target.files[0])
                                    }" />
                                </div>
                                <div class="text-right my-3">
                                    <button data-show=".payment"
                                        class="btn w-btn-2 reverse_hover primary-color box-shadow-none navigation-btn"
                                        @click="handleSumit">Submit</button>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-body text-center">
                    <img style="max-width: 100%;" :src="previewImg" id="preview_box" alt="">
                </div>
